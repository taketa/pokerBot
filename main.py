...
while True:
	...
	def lastHand(textx=pyperclip.paste().split('\n')):
		lastHand=[]
		for line in textx:
			if '#' in line:
				lastHand.append(textx.index(line))
		if len(lastHand)>=1:
			lastHand=textx[max(lastHand):]			#определение последней руки
		...
		if len (lastHand)>3:
			...
			if ' ' in nic:
				textt=[x.replace(nic,nic2) for x in textt]
...
######################################################################
	if len(lastHand())>3:
		if 'finishes in' not in lastHand()[-1] and 'stands up' not in lastHand()[-1]:
			...
			class bord:
				def bord(self,last=lastHand(pyperclip.paste().split('\n'))):
					def flopCards():
						for line in last:
			...
			def rounddd(x):			#определение текущего раунда
				doska=bord()
				doska=doska.bord(x)
			...
			def didI(last=pyperclip.paste().split('\n')):
				pos=position()
				if pos:
					if len(last)>=3:
...
################################################################
			...
			while rounddd(lastHand(pyperclip.paste().split('\n')))==0 and lastHand(pyperclip.paste().split('\n'))[0].split(' ')[-1]==indicator:
				print(rounddd(lastHand(pyperclip.paste().split('\n'))))
				time.sleep(5)
				chattx()
				if didI(lastHand(pyperclip.paste().split('\n'))):
					diapazonx=diapazonPreFlop()
					if lineIP:
						try:
							eval(lineIP)
						except NameError:
							diapazonx=diapazonPercent(diapazonx,0.33)
						else:
							diapazonx=diapazonPercent(diapazonx,eval(lineIP))
					if position()=='oop':

						try:eval(lineIP)
						except NameError:
							print(evaluatePreFlop(diapazonx))
							if lineIP and lineIP[-1]=='c':
								if evaluatePreFlop(diapazonx)>=0.55:bettx()
								else:checkkx()
							elif lineIP and lineIP[-1]=='r':

								if evaluatePreFlop(diapazonx)>=0.75:

									if stack<=toCallx:calllx()
									bettx()

								elif evaluatePreFlop(diapazonx)>=toCallx/(potx+toCallx):calllx()
								foldsx()
								
									
						else:
							print(evaluatePreFlop(diapazonx.diapazonPercent(diapazonx,eval(lineIP))))
							if lineIP and lineIP[-1]=='c':
								if evaluatePreFlop(diapazonx.diapazonPercent(diapazonx,eval(lineIP)))>=0.55:bettx()
								else:checkkx()
							elif lineIP and lineIP[-1]=='r':
								if evaluatePreFlop(diapazonPercent(diapazonx,eval(lineIP)))>=0.75:
									if stack<=toCallx:calllx()
									bettx()
								elif evaluatePreFlop(diapazonPercent(diapazonx,eval(lineIP)))>=toCallx/(potx+toCallx):calllx()
								foldsx()
					if position()=='ip':
						if not lineIP:
							print(evaluatePreFlop(diapazonx))
							...
							try:eval(lineIP)
								...
								print(evaluatePreFlop(diapazonPercent(diapazonx,eval(lineIP))))
								if lineIP and lineIP[-1]=='r':
									if evaluatePreFlop(diapazonPercent(diapazonx,eval(lineIP)))>=0.75:
										if stack<=toCallx:calllx()
										pushhx()
									...
									foldsx()
			while rounddd(lastHand(pyperclip.paste().split('\n')))==1 and lastHand(pyperclip.paste().split('\n'))[0].split(' ')[-1]==indicator:
				print('ya tut')
				time.sleep(5)
				chattx()
				if didI(lastHand(pyperclip.paste().split('\n'))):
					...
					if lineIP:
						try:eval(lineIP)
							...
							diapazon1=diapazonPercent(diapazon1,0.33)
						else:
							diapazon1=diapazonPercent(diapazon1,eval(lineIP))
					# print(evaluatePostFlop(diapazonPercent(diapazon1,eval(lineIP))))
					print(lineIP)
					if lineIP[-1]!='r' and lineIP[-1]!='b':
						if evaluatePostFlop(diapazonPercent(diapazon1,eval(lineIP)))>=0.5:bettx()
						checkkx()
					
					else:
						if evaluatePostFlop(diapazonPercent(diapazon1,eval(lineIP)))>=0.75:
							if stack<=toCallx:calllx()
							pushhx()
						elif evaluatePostFlop(diapazonPercent(diapazon1,eval(lineIP)))>=toCallx/(potx+toCallx):calllx()
						foldsx()
			while rounddd(lastHand(pyperclip.paste().split('\n')))==2 and lastHand(pyperclip.paste().split('\n'))[0].split(' ')[-1]==indicator:
				time.sleep(5)
				chattx()
				if didI(lastHand(pyperclip.paste().split('\n'))):
					diapazon2=diapazon1
					if lineIP:
						try:eval(lineIP)
							...
							diapazon2=diapazonPercent(diapazon2,0.33)
						else:
							diapazon2=diapazonPercent(diapazon2,eval(lineIP))
					print(evaluatePostFlop(diapazonPercent(diapazon2,eval(lineIP))))
					if lineIP[-1]!='r' and lineIP[-1]!='b':
						if evaluatePostFlop(diapazonPercent(diapazon2,eval(lineIP)))>=0.5:bettx()
						checkkx()
					
					else:
						if evaluatePostFlop(diapazonPercent(diapazon2,eval(lineIP)))>=0.75:
							if stack<=toCallx:calllx()
							pushhx()
						elif evaluatePostFlop(diapazonPercent(diapazon2,eval(lineIP)))>=toCallx/(potx+toCallx):calllx()
						foldsx()
			while rounddd(lastHand(pyperclip.paste().split('\n')))==3 and lastHand(pyperclip.paste().split('\n'))[0].split(' ')[-1]==indicator:
				time.sleep(5)
				chattx()
				if didI(lastHand(pyperclip.paste().split('\n'))):
					diapazon3=diapazon2
					if lineIP:
						try:eval(lineIP)
							...
							diapazon3=diapazonPercent(diapazon3,0.33)
						else:
							diapazon3=diapazonPercent(diapazon3,eval(lineIP))
					print(evaluatePostFlop(diapazonPercent(diapazon3,eval(lineIP))))
					if lineIP[-1]!='r' and lineIP[-1]!='b':
						if evaluatePostFlop(diapazonPercent(diapazon3,eval(lineIP)))>=0.5:bettx()
						checkkx()
					
					else:
						if evaluatePostFlop(diapazonPercent(diapazon3,eval(lineIP)))>=0.75:
							if stack<=toCallx:calllx()
							pushhx()
						elif evaluatePostFlop(diapazonPercent(diapazon3,eval(lineIP)))>=toCallx/(potx+toCallx):calllx()
						foldsx()
