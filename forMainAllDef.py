...
if len(lastHand())>3:
	if 'finishes in' not in lastHand()[-1] and 'stands up' not in lastHand()[-1]:
		...
		if len(c_OOPx)>=3:
			c_OOPx=c_OOPx[-3:]
			c0_OOP=sum([1 for x in c_OOPx if 'c0' in x[:2]])/float(3)
		crcbcbf_OOP=0.5
		crcbcb_OOPx=[x for x in statsOOP if 'crcbcb' in x[:6]]
		if len(crcbcb_OOPx)>=3:
			crcbcb_OOPx=crcbcb_OOPx[-3:]
			crcbcbf_OOP=sum([1 for x in crcbcb_OOPx if 'crcbcbf' in x[:7]])/float(3)
		crcbf_OOP=0.5
		rrcbc_OOP=0.5
		crcb_OOPx=[x for x in statsOOP if 'crcb' in x[:4]]
		if len(crcb_OOPx)>=3:
			crcb_OOPx=crcb_OOPx[-3:]
			crcbf_OOP=sum([1 for x in crcb_OOPx if 'crcbf' in x[:5]])/float(3)
			rrcbc_OOP=sum([1 for x in crcb_OOPx if 'crcbc' in x[:5]])/float(3)
		crf_OOP=0.5
		crr_OOP=0.5
		cr_OOPx=[x for x in statsOOP if 'cr' in x[:2]]
		if len(cr_OOPx)>=3:
			cr_OOPx=cr_OOPx[-3:]
			crf_OOP=sum([1 for x in cr_OOPx if 'crf' in x[:3]])/float(3)
			crr_OOP=sum([1 for x in cr_OOPx if 'cr' in x[:3]])/float(3)
		rrf_OOP=0.5
		rr_OOPx=[x for x in statsOOP if 'rr' in x[:2]]
		if len(rr_OOPx)>=3:
			rr_OOPx=rr_OOPx[-3:]
			rrf_OOP=sum([1 for x in rr_OOPx if 'rrf' in x[:3]])/float(3)
		rrcbf_OOP=0.5
		rrcb_OOPx=[x for x in statsOOP if 'rrcb' in x[:4]]
		if len(rrcb_OOPx)>=3:
			rrcb_OOPx=rrcb_OOPx[-3:]
			rrcbf_OOP=sum([1 for x in rrcb_OOPx if 'rrcbf' in x[:5]])/float(3)
		rcbf_OOP=0.5
		rcb_OOPx=[x for x in statsOOP if 'rcb' in x[:3]]
		if len(rcb_OOPx)>=3:
			rcb_OOPx=rcb_OOPx[-3:]
			rcbf_OOP=sum([1 for x in rcb_OOPx if 'rcbf' in x[:4]])/float(3)
		rc0brf_OOP=0.5
		rc0br_OOPx=[x for x in statsOOP if 'rc0br' in x[:5]]
		if len(rc0br_OOPx)>=3:
			rc0br_OOPx=rc0br_OOPx[-3:]
			rc0brf_OOP=sum([1 for x in rc0br_OOPx if 'rc0brf' in x[:6]])/float(3)
		rc00bf_OOP=0.5
		rc00b_OOPx=[x for x in statsOOP if 'rc00b' in x[:5]]
		if len(rc00b_OOPx)>=3:
			rc00b_OOPx=rc00b_OOPx[-3:]
			rc00bf_OOP=sum([1 for x in rc00b_OOPx if 'rc00bf' in x[:6]])/float(3)
		rc00bcbf_OOP=0.5
		rc00bcb_OOPx=[x for x in statsOOP if 'rc00bcb' in x[:7]]
		if len(rc00bcb_OOPx)>=3:
			rc00bcb_OOPx=rc00bcb_OOPx[-3:]
			rc00bcbf_OOP=sum([1 for x in rc00bcb_OOPx if 'rc00bcbf' in x[:8]])/float(3)
		c0bf_OOP=0.5
		c0b_OOPx=[x for x in statsOOP if 'c0b' in x[:3]]
		if len(c0b_OOPx)>=3:
			c0b_OOPx=c0b_OOPx[-3:]
			c0bf_OOP=sum([1 for x in c0b_OOPx if 'c0bf' in x[:4]])/float(3)
		c0bcbf_OOP=0.5
		c0bcb_OOPx=[x for x in statsOOP if 'c0bcb' in x[:5]]
		if len(c0bcb_OOPx)>=3:
			c0bcb_OOPx=c0bcb_OOPx[-3:]
			c0bcbf_OOP=sum([1 for x in c0bcb_OOPx if 'c0bcbf' in x[:6]])/float(3)
		c00bf_OOP=0.5
		c00b_OOPx=[x for x in statsOOP if 'c00b' in x[:4]]
		if len(c00b_OOPx)>=3:
			c00b_OOPx=c00b_OOPx[-3:]
			c00bf_OOP=sum([1 for x in c00b_OOPx if 'c00bf' in x[:5]])/float(3)
		rf_OOP=0.5
		r_OOPx=[x for x in statsOOP if 'r' in x[:1]]
		if len(r_OOPx)>=3:
			r_OOPx=r_OOPx[-3:]
			rf_OOP=sum([1 for x in r_OOPx if 'rf' in x[:5]])/float(3)
		rrc_OOP=0.5
		rr_OOPx=[x for x in statsOOP if 'rr' in x[:2]]
		if len(rr_OOPx)>=3:
			rr_OOPx=rr_OOPx[-3:]
			rrc_OOP=sum([1 for x in rr_OOPx if 'rrc' in x[:3]])/float(3)
		crc_OOP=0.5
		cr_OOPx=[x for x in statsOOP if 'cr' in x[:2]]
		if len(cr_OOPx)>=3:
			cr_OOPx=cr_OOPx[-3:]
			crc_OOP=sum([1 for x in cr_OOPx if 'crc' in x[:3]])/float(3)
		crcbc_OOP=0.5
		crcb_OOPx=[x for x in statsOOP if 'crcb' in x[:4]]
		if len(crcb_OOPx)>=3:
			crcb_OOPx=crcb_OOPx[-3:]
			crcbc_OOP=sum([1 for x in crcb_OOPx if 'crcbc' in x[:5]])/float(3)
		

		r_OOP=[x for x in statsOOP]
		if len(r_OOP)>=3:
			r_OOPx=r_OOPx[-3:]
			r_OOP=sum([1 for x in r_OOPx if 'r' in x[:1]])/float(3)
		rrr_OOP=0.5
		rr_OOPx=[x for x in statsOOP]
		if len(rr_OOPx)>=3:
			rr_OOPx=rr_OOPx[-3:]
			rrr_OOP=sum([1 for x in rr_OOPx if 'rrr' in x[:3]])/float(3)
		rrcbcb_OOP=0.5
		rrcbc_OOPx=[x for x in statsOOP if 'rrcbc' in x[:5]]
		if len(rrcbc_OOPx)>=3:
			rrcbc_OOPx=rrcbc_OOPx[-3:]
			rrcbcb_OOP=sum([1 for x in rrcbc_OOPx if 'rrcbcb' in x[:6]])/float(3)
		rrcbr_OOP=0.5
		rrcb_OOPx=[x for x in statsOOP if 'rrcb' in x[:4]]
		if len(rrcb_OOPx)>=3:
			rrcb_OOPx=rrcb_OOPx[-3:]
			rrcbr_OOP=sum([1 for x in rrcb_OOPx if 'rrcbr' in x[:5]])/float(3)
		rrcbcbr_OOP=0.5
		rrcbcb_OOPx=[x for x in statsOOP if 'rrcbcb' in x[:6]]
		if len(rrcbcb_OOPx)>=3:
			rrcbcb_OOPx=rrcbcb_OOPx[-3:]
			rrcbcbr_OOP=sum([1 for x in rrcbcb_OOPx if 'rrcbcbr' in x[:7]])/float(3)
		rrcbcbc_OOP=0.5
		rrcbcb_OOPx=[x for x in statsOOP if 'rrcbcb' in x[:6]]
		if len(rrcbcb_OOPx)>=3:
			rrcbcb_OOPx=rrcbcb_OOPx[-3:]
			rrcbcbc_OOP=sum([1 for x in rrcbcb_OOPx if 'rrcbcbc' in x[:7]])/float(3)
		rrcbcbf_OOP=0.5
		rrcbcb_OOPx=[x for x in statsOOP if 'rrcbcb' in x[:6]]
		if len(rrcbcb_OOPx)>=3:
			rrcbcb_OOPx=rrcbcb_OOPx[-3:]
			rrcbcbf_OOP=sum([1 for x in rrcbcb_OOPx if 'rrcbcbf' in x[:7]])/float(3)
		rrcbcbcbf_OOP=0.5
		rrcbcbcb_OOPx=[x for x in statsOOP if 'rrcbcbcb' in x[:8]]
		if len(rrcbcbcb_OOPx)>=3:
			rrcbcbcb_OOPx=rrcbcbcb_OOPx[-3:]
			rrcbcbcbf_OOP=sum([1 for x in rrcbcbcb_OOPx if 'rrcbcbcbf' in x[:9]])/float(3)
		c_OOP=0.5
		c_OOPx=[x for x in statsOOP]
		if len(c_OOPx)>=3:
			c_OOPx=c_OOPx[-3:]
			c_OOP=sum([1 for x in c_OOPx if 'c' in x[:1]])/float(3)
		...
		if len(rx)>=3:
			rx=rx[-3:]
			rf=sum([1 for x in rx if 'rf' in x[:2]])/float(3)
			rr=sum([1 for x in rx if 'rr' in x[:2]])/float(3)
			rc=sum([1 for x in rx if 'rc' in x[:2]])/float(3)
		...
		if len(rcbrx)>=3:
			rcbrx=rcbrx[-3:]
			rcbrr=sum([1 for x in rcbrx if 'rcbrr' in x[:5]])/float(3)
		...
		if len(rcbrx)>=3:
			rcbrx=rcbrx[-3:]
			rcbrf=sum([1 for x in rcbrx if 'rcbrf' in x[:5]])/float(3)
		rcb=0.5
		rcx=[x for x in statsIP if 'rc' in x[:2]]
		if len(rcx)>=3:
			rcx=rcx[-3:]
			rcb=sum([1 for x in rcx if 'rcb' in x[:3]])/float(3)
		rc0=0.5
		rcx=[x for x in statsIP if 'rc' in x[:2]]
		if len(rcx)>=3:
			rcx=rcx[-3:]
			rc0=sum([1 for x in rcx if 'rc0' in x[:3]])/float(3)
		rc0bc=0.5
		rc0bx=[x for x in statsIP if 'rc0b' in x[:4]]
		if len(rc0bx)>=3:
			rc0bx=rc0bx[-3:]
			rc0bc=sum([1 for x in rc0bx if 'rc0bc' in x[:5]])/float(3)

		rc0bf=0.5
		rc0bx=[x for x in statsIP if 'rc0b' in x[:4]]
		if len(rc0bx)>=3:
			rc0bx=rc0bx[-3:]
			rc0bf=sum([1 for x in rc0bx if 'rc0bf' in x[:5]])/float(3)
		rc0bc0bf=0.5
		rc0bc0bx=[x for x in statsIP if 'rc0bc0b' in x[:7]]
		if len(rc0bc0bx)>=3:
			rc0bc0bx=rc0bc0bx[-3:]
			rc0bc0bf=sum([1 for x in rc0bc0bx if 'rc0bc0bf' in x[:8]])/float(3)
		rrcbrf=0.5
		rrcbrx=[x for x in statsIP if 'rrcbr' in x[:5]]
		if len(rrcbrx)>=3:
			rrcbrx=rrcbrx[-3:]
			rrcbrf=sum([1 for x in rrcbrx if 'rrcbrf' in x[:6]])/float(3)
		rrc0bf=0.5
		rrc0bx=[x for x in statsIP if 'rrc0b' in x[:5]]
		if len(rrc0bx)>=3:
			rrc0bx=rrc0bx[-3:]
			rrc0bf=sum([1 for x in rrc0bx if 'rrc0bf' in x[:6]])/float(3)
		rrc0bc0bf=0.5
		rrc0bc0bx=[x for x in statsIP if 'rrc0bc0b' in x[:8]]
		if len(rrc0bc0bx)>=3:
			rrc0bc0bx=rrc0bc0bx[-3:]
			rrc0bc0bf=sum([1 for x in rrc0bc0bx if 'rrc0bc0bf' in x[:9]])/float(3)
		rc0bc0=0.5
		rc0bcx=[x for x in statsIP if 'rc0bc' in x[:5]]
		if len(rc0bcx)>=3:
			rc0bcx=rc0bcx[-3:]
			rc0bc0=sum([1 for x in rc0bcx if 'rc0bc0' in x[:6]])/float(3)
		rc0bc0bc=0.5
		rc0bc0bx=[x for x in statsIP if 'rc0bc0b' in x[:7]]
		if len(rc0bc0bx)>=3:
			rc0bc0bx=rc0bc0bx[-3:]
			rc0bc0bc=sum([1 for x in rc0bc0bx if 'rc0bc0bc' in x[:8]])/float(3)
		rc0br=0.5
		rc0bx=[x for x in statsIP if 'rc0b' in x[:4]]
		if len(rc0bx)>=3:
			rc0bx=rc0bx[-3:]
			rc0br=sum([1 for x in rc0bx if 'rc0br' in x[:5]])/float(3)
		rc0bc0br=0.5
		rc0bc0bx=[x for x in statsIP if 'rc0bc0b' in x[:7]]
		if len(rc0bc0bx)>=3:
			rc0bc0bx=rc0bc0bx[-3:]
			rc0bc0br=sum([1 for x in rc0bc0bx if 'rc0bc0br' in x[:8]])/float(3)
		rc0b=0.5
		rc0x=[x for x in statsIP if 'rc0' in x[:3]]
		if len(rc0x)>=3:
			rc0x=rc0x[-3:]
			rc0b=sum([1 for x in rc0x if 'rc0b' in x[:4]])/float(3)
...
################################################################
		...
		def diapazonPreFlop():
			...
			if myCardss:
				...
				return diapazon
		def diapazonMinus(koloda):
			kolNew=[(x[:2],x[2:]) for x in koloda]
			kolNew=[','.join(x) for x in kolNew]
			kolNew=','.join(kolNew)
			kolNew=kolNew.split(',')
			kolNew=set(kolNew)
			kolNew=list(kolNew)
			kolNew=sorted(kolNew,reverse=True)

			# print(kolNew)

			if myCardss:
				if myCardss[0] in kolNew:kolNew.remove(myCardss[0])
				if myCardss[1] in kolNew:kolNew.remove(myCardss[1])
				for x in bordx:
					if x in kolNew:kolNew.remove(x)
				diapazon=[x+kolNew[i] for x in kolNew for i in range(kolNew.index(x)+1,len(kolNew))]
				diapazonPare=[x for x in diapazon if x[0]==x[2]]
				diapazonNonPare=[x for x in diapazon if x[0]!=x[2]]
				diapazonx=diapazonPare+diapazonNonPare
				diapazon=diapazonPostFlop(diapazonx)
				return diapazon
		...
		def diapazonPostFlop(diapazon):
			...
			diapazonDel=[diapazonKare.remove(x) for x in diapazonKare if x in diapazon]
			diapazonFlash=[flash(x) for x in diapazon]
			diapazonFlash=[x for x in diapazonFlash if x]
			diapazonDel=[diapazonFlash.remove(x) for x in diapazonFlash if x in diapazon]
			diapazonStreet=[street(x) for x in diapazon]
			diapazonStreet=[x for x in diapazonStreet if x]
			diapazonDel=[diapazonStreet.remove(x) for x in diapazonStreet if x in diapazon]
			diapazonTrips=[trips(x) for x in diapazon]
			diapazonTrips=[x for x in diapazonTrips if x]
			diapazonDel=[diapazonTrips.remove(x) for x in diapazonTrips if x in diapazon]
			diapazonTwoPare=[twopare(x) for x in diapazon]
			diapazonTwoPare=[x for x in diapazonTwoPare if x]
			diapazonDel=[diapazonTwoPare.remove(x) for x in diapazonTwoPare if x in diapazon]
			diapazonPare=[pare(x) for x in diapazon]
			diapazonPare=[x for x in diapazonPare if x]
			diapazonDel=[diapazonPare.remove(x) for x in diapazonPare if x in diapazon]
		...
		def evaluatePreFlop(diapazonx):
			...
			for x in diapazonx:
				evaluatex.append(enum([str(''.join(myCardss)), x], trials=100))
			[sumEvaluate.append(x[0]) for x in evaluatex]
			evaluatex=sum(sumEvaluate)/float(len(sumEvaluate))
		...
		def evaluatePostFlop(diapazonx):
			...
			for x in diapazonx:
				evaluatex.append(enum([str(''.join(myCardss)), x],str(''.join(bordx))))
				# print(x+'	',enum([str(''.join(myCardss)), x],str(''.join(bordx))))
			[sumEvaluate.append(x[0]) for x in evaluatex]
			evaluatex=sum(sumEvaluate)/float(len(sumEvaluate))
