	...
	######################################################################
		if len(lastHand())>3:
			if 'finishes in' not in lastHand()[-1] and 'stands up' not in lastHand()[-1]:
				...
				if len(c_OOPx)>=2:
					c_OOPx=c_OOPx[-2:]
					c0_OOP=sum([1 for x in c_OOPx if 'c0' in x[:2]])
				crcbcbf_OOP=0
				crcbcb_OOPx=[x for x in statsOOP if 'crcbcb' in x[:6]]
				if len(crcbcb_OOPx)>=2:
					crcbcb_OOPx=crcbcb_OOPx[-2:]
					crcbcbf_OOP=sum([1 for x in crcbcb_OOPx if 'crcbcbf' in x[:7]])
				crcbf_OOP=0
				rrcbc_OOP=0
				crcb_OOPx=[x for x in statsOOP if 'crcb' in x[:4]]
				if len(crcb_OOPx)>=2:
					crcb_OOPx=crcb_OOPx[-2:]
					crcbf_OOP=sum([1 for x in crcb_OOPx if 'crcbf' in x[:5]])
					rrcbc_OOP=sum([1 for x in crcb_OOPx if 'crcbc' in x[:5]])
				crf_OOP=0
				crr_OOP=0
				cr_OOPx=[x for x in statsOOP if 'cr' in x[:2]]
				if len(cr_OOPx)>=2:
					cr_OOPx=cr_OOPx[-2:]
					crf_OOP=sum([1 for x in cr_OOPx if 'crf' in x[:3]])
					crr_OOP=sum([1 for x in cr_OOPx if 'cr' in x[:3]])
				rrf_OOP=0
				rr_OOPx=[x for x in statsOOP if 'rr' in x[:2]]
				if len(rr_OOPx)>=2:
					rr_OOPx=rr_OOPx[-2:]
					rrf_OOP=sum([1 for x in rr_OOPx if 'rrf' in x[:3]])
				rrcbf_OOP=0
				rrcb_OOPx=[x for x in statsOOP if 'rrcb' in x[:4]]
				if len(rrcb_OOPx)>=2:
					rrcb_OOPx=rrcb_OOPx[-2:]
					rrcbf_OOP=sum([1 for x in rrcb_OOPx if 'rrcbf' in x[:5]])
				rcbf_OOP=0
				rcb_OOPx=[x for x in statsOOP if 'rcb' in x[:3]]
				if len(rcb_OOPx)>=2:
					rcb_OOPx=rcb_OOPx[-2:]
					rcbf_OOP=sum([1 for x in rcb_OOPx if 'rcbf' in x[:4]])
				rc0brf_OOP=0
				rc0br_OOPx=[x for x in statsOOP if 'rc0br' in x[:5]]
				if len(rc0br_OOPx)>=2:
					rc0br_OOPx=rc0br_OOPx[-2:]
					rc0brf_OOP=sum([1 for x in rc0br_OOPx if 'rc0brf' in x[:6]])
				rc00bf_OOP=0
				rc00b_OOPx=[x for x in statsOOP if 'rc00b' in x[:5]]
				if len(rc00b_OOPx)>=2:
					rc00b_OOPx=rc00b_OOPx[-2:]
					rc00bf_OOP=sum([1 for x in rc00b_OOPx if 'rc00bf' in x[:6]])
				rc00bcbf_OOP=0
				rc00bcb_OOPx=[x for x in statsOOP if 'rc00bcb' in x[:7]]
				if len(rc00bcb_OOPx)>=2:
					rc00bcb_OOPx=rc00bcb_OOPx[-2:]
					rc00bcbf_OOP=sum([1 for x in rc00bcb_OOPx if 'rc00bcbf' in x[:8]])
				c0bf_OOP=0
				c0b_OOPx=[x for x in statsOOP if 'c0b' in x[:3]]
				if len(c0b_OOPx)>=2:
					c0b_OOPx=c0b_OOPx[-2:]
					c0bf_OOP=sum([1 for x in c0b_OOPx if 'c0bf' in x[:4]])
				c0bcbf_OOP=0
				c0bcb_OOPx=[x for x in statsOOP if 'c0bcb' in x[:5]]
				if len(c0bcb_OOPx)>=2:
					c0bcb_OOPx=c0bcb_OOPx[-2:]
					c0bcbf_OOP=sum([1 for x in c0bcb_OOPx if 'c0bcbf' in x[:6]])
				c00bf_OOP=0
				c00b_OOPx=[x for x in statsOOP if 'c00b' in x[:4]]
				if len(c00b_OOPx)>=2:
					c00b_OOPx=c00b_OOPx[-2:]
					c00bf_OOP=sum([1 for x in c00b_OOPx if 'c00bf' in x[:5]])
				rf_OOP=0
				r_OOPx=[x for x in statsOOP if 'r' in x[:1]]
				if len(r_OOPx)>=2:
					r_OOPx=r_OOPx[-2:]
					rf_OOP=sum([1 for x in r_OOPx if 'rf' in x[:5]])
				rrc_OOP=0
				rr_OOPx=[x for x in statsOOP if 'rr' in x[:2]]
				if len(rr_OOPx)>=2:
					rr_OOPx=rr_OOPx[-2:]
					rrc_OOP=sum([1 for x in rr_OOPx if 'rrc' in x[:3]])
				crc_OOP=0
				cr_OOPx=[x for x in statsOOP if 'cr' in x[:2]]
				if len(cr_OOPx)>=2:
					cr_OOPx=cr_OOPx[-2:]
					crc_OOP=sum([1 for x in cr_OOPx if 'crc' in x[:3]])
				crcbc_OOP=0
				crcb_OOPx=[x for x in statsOOP if 'crcb' in x[:4]]
				if len(crcb_OOPx)>=2:
					crcb_OOPx=crcb_OOPx[-2:]
					crcbc_OOP=sum([1 for x in crcb_OOPx if 'crcbc' in x[:5]])
				

				r_OOP=[x for x in statsOOP]
				if len(r_OOP)>=2:
					r_OOPx=r_OOPx[-2:]
					r_OOP=sum([1 for x in r_OOPx if 'r' in x[:1]])
				rrr_OOP=0
				rr_OOPx=[x for x in statsOOP]
				if len(rr_OOPx)>=2:
					rr_OOPx=rr_OOPx[-2:]
					rrr_OOP=sum([1 for x in rr_OOPx if 'rrr' in x[:3]])
				rrcbcb_OOP=0
				rrcbc_OOPx=[x for x in statsOOP if 'rrcbc' in x[:5]]
				if len(rrcbc_OOPx)>=2:
					rrcbc_OOPx=rrcbc_OOPx[-2:]
					rrcbcb_OOP=sum([1 for x in rrcbc_OOPx if 'rrcbcb' in x[:6]])
				rrcbr_OOP=0
				rrcb_OOPx=[x for x in statsOOP if 'rrcb' in x[:4]]
				if len(rrcb_OOPx)>=2:
					rrcb_OOPx=rrcb_OOPx[-2:]
					rrcbr_OOP=sum([1 for x in rrcb_OOPx if 'rrcbr' in x[:5]])
				rrcbcbr_OOP=0
				rrcbcb_OOPx=[x for x in statsOOP if 'rrcbcb' in x[:6]]
				if len(rrcbcb_OOPx)>=2:
					rrcbcb_OOPx=rrcbcb_OOPx[-2:]
					rrcbcbr_OOP=sum([1 for x in rrcbcb_OOPx if 'rrcbcbr' in x[:7]])
				rrcbcbc_OOP=0
				rrcbcb_OOPx=[x for x in statsOOP if 'rrcbcb' in x[:6]]
				if len(rrcbcb_OOPx)>=2:
					rrcbcb_OOPx=rrcbcb_OOPx[-2:]
					rrcbcbc_OOP=sum([1 for x in rrcbcb_OOPx if 'rrcbcbc' in x[:7]])
				rrcbcbf_OOP=0
				rrcbcb_OOPx=[x for x in statsOOP if 'rrcbcb' in x[:6]]
				if len(rrcbcb_OOPx)>=2:
					rrcbcb_OOPx=rrcbcb_OOPx[-2:]
					rrcbcbf_OOP=sum([1 for x in rrcbcb_OOPx if 'rrcbcbf' in x[:7]])
				rrcbcbcbf_OOP=0
				rrcbcbcb_OOPx=[x for x in statsOOP if 'rrcbcbcb' in x[:8]]
				if len(rrcbcbcb_OOPx)>=2:
					rrcbcbcb_OOPx=rrcbcbcb_OOPx[-2:]
					rrcbcbcbf_OOP=sum([1 for x in rrcbcbcb_OOPx if 'rrcbcbcbf' in x[:9]])
				c_OOP=0
				c_OOPx=[x for x in statsOOP]
				if len(c_OOPx)>=2:
					c_OOPx=c_OOPx[-2:]
					c_OOP=sum([1 for x in c_OOPx if 'c' in x[:1]])
				...
				if len(rx)>=2:
					rx=rx[-2:]
					rf=sum([1 for x in rx if 'rf' in x[:2]])
					rr=sum([1 for x in rx if 'rr' in x[:2]])
					rc=sum([1 for x in rx if 'rc' in x[:2]])
				...
				if len(rcbrx)>=2:
					rcbrx=rcbrx[-2:]
					rcbrr=sum([1 for x in rcbrx if 'rcbrr' in x[:5]])
				...
				if len(rcbrx)>=2:
					rcbrx=rcbrx[-2:]
					rcbrf=sum([1 for x in rcbrx if 'rcbrf' in x[:5]])
				rcb=0
				rcx=[x for x in statsIP if 'rc' in x[:2]]
				if len(rcx)>=2:
					rcx=rcx[-2:]
					rcb=sum([1 for x in rcx if 'rcb' in x[:3]])
				rc0=0
				rcx=[x for x in statsIP if 'rc' in x[:2]]
				if len(rcx)>=2:
					rcx=rcx[-2:]
					rc0=sum([1 for x in rcx if 'rc0' in x[:3]])
				rc0bc=0
				rc0bx=[x for x in statsIP if 'rc0b' in x[:4]]
				if len(rc0bx)>=2:
					rc0bx=rc0bx[-2:]
					rc0bc=sum([1 for x in rc0bx if 'rc0bc' in x[:5]])

				rc0bf=0
				rc0bx=[x for x in statsIP if 'rc0b' in x[:4]]
				if len(rc0bx)>=2:
					rc0bx=rc0bx[-2:]
					rc0bf=sum([1 for x in rc0bx if 'rc0bf' in x[:5]])
				rc0bc0bf=0
				rc0bc0bx=[x for x in statsIP if 'rc0bc0b' in x[:7]]
				if len(rc0bc0bx)>=2:
					rc0bc0bx=rc0bc0bx[-2:]
					rc0bc0bf=sum([1 for x in rc0bc0bx if 'rc0bc0bf' in x[:8]])
				rrcbrf=0
				rrcbrx=[x for x in statsIP if 'rrcbr' in x[:5]]
				if len(rrcbrx)>=2:
					rrcbrx=rrcbrx[-2:]
					rrcbrf=sum([1 for x in rrcbrx if 'rrcbrf' in x[:6]])
				rrc0bf=0
				rrc0bx=[x for x in statsIP if 'rrc0b' in x[:5]]
				if len(rrc0bx)>=2:
					rrc0bx=rrc0bx[-2:]
					rrc0bf=sum([1 for x in rrc0bx if 'rrc0bf' in x[:6]])
				rrc0bc0bf=0
				rrc0bc0bx=[x for x in statsIP if 'rrc0bc0b' in x[:8]]
				if len(rrc0bc0bx)>=2:
					rrc0bc0bx=rrc0bc0bx[-2:]
					rrc0bc0bf=sum([1 for x in rrc0bc0bx if 'rrc0bc0bf' in x[:9]])
				rc0bc0=0
				rc0bcx=[x for x in statsIP if 'rc0bc' in x[:5]]
				if len(rc0bcx)>=2:
					rc0bcx=rc0bcx[-2:]
					rc0bc0=sum([1 for x in rc0bcx if 'rc0bc0' in x[:6]])
				rc0bc0bc=0
				rc0bc0bx=[x for x in statsIP if 'rc0bc0b' in x[:7]]
				if len(rc0bc0bx)>=2:
					rc0bc0bx=rc0bc0bx[-2:]
					rc0bc0bc=sum([1 for x in rc0bc0bx if 'rc0bc0bc' in x[:8]])
				rc0br=0
				rc0bx=[x for x in statsIP if 'rc0b' in x[:4]]
				if len(rc0bx)>=2:
					rc0bx=rc0bx[-2:]
					rc0br=sum([1 for x in rc0bx if 'rc0br' in x[:5]])
				rc0bc0br=0
				rc0bc0bx=[x for x in statsIP if 'rc0bc0b' in x[:7]]
				if len(rc0bc0bx)>=2:
					rc0bc0bx=rc0bc0bx[-2:]
					rc0bc0br=sum([1 for x in rc0bc0bx if 'rc0bc0br' in x[:8]])
				rc0b=0
				rc0x=[x for x in statsIP if 'rc0' in x[:3]]
				if len(rc0x)>=2:
					rc0x=rc0x[-2:]
					rc0b=sum([1 for x in rc0x if 'rc0b' in x[:4]])
				...
				def ActionMyFin():
					if position()=='oop':
						if lineIP and lineIP[-1]=='c':
							...
							if rounddd()==0:
								if (effectiveStack()/potx)<=8:
									if preflopCards in push:return 'push'

								if preflopCards in push and crr_OOP<2:
									if effectiveStack()>=0.5*potx:return 'raise'
									return 'push'
								if preflopCards in call and crr_OOP>1:
									if effectiveStack()>=0.5*potx:return 'raise'
									return 'push'
								if preflopCards in powerCall or preflopCards in call:return 'raise'
							if rounddd()==1:
								if cards in nice:
									if 'is all in' in last[-1]:return 'call'
									if effectiveStack()>=0.5*potx:return 'raise'
									return 'push'
							if rounddd()>1:
								if cards in nice:
									...
									if effectiveStack()>=0.5*potx:return 'raise'
									return 'push'
							# print([1 for x,y in statsOOPMas if lineIP+'bf' in x[:len(lineIP+'bf')] and y>0])
							# print([1 for x,y in statsOOPMas if lineIP+'b' in x and len(x)==len(lineIP+'b') and not y])
							if [1 for x,y in statsOOPMas if lineIP+'rf' in x[:len(lineIP+'bf')] and type(y)==type(1) and y>1]:
								if rounddd()==0:
									if effectiveStack()>=potx:return 'raise'
									return 'push'
							if [1 for x,y in statsOOPMas if lineIP+'bf' in x[:len(lineIP+'bf')] and type(y)==type(1) and y>0]:
								if effectiveStack()>=0.5*potx:return 'raise'
								return 'push'
							
							
								# if [1 for x,y in statsIPMas if lineIP+'rcbf' in x[:len(lineIP+'rcbf')] and y>0] or [1 for x,y in statsIPMas if lineIP+'rcb' in x[:len(lineIP+'rcb')] and y<3]:
								# 	if effectiveStack()>=2.5*potx:return 'raise'
								# 	return 'push'
							if [1 for x,y in statsOOPMas if lineIP in x[:len(lineIP)] and type(y)==type(1) and y>1] and preflopCards in raiss and rounddd==0:
								if effectiveStack()>=potx and 'is all in' not in last[-1]:return 'raise'

								return 'push'
							if [1 for x,y in statsOOPMas if lineIP in x[:len(lineIP)] and type(y)==type(1) and y>1] and cards in nice and rounddd>0:
								if effectiveStack()>=0.5*potx and 'is all in' not in last[-1]:return 'raise'
						...
						if lineIP and lineIP[-1]=='r':
							if rounddd()==0:
								...
								if toCallx/(potx-toCallx)<=4.5:
									if (effectiveStack()/potx)<=8:
										if preflopCards in call and 'is all in' in last[-1]:return 'call'
										if preflopCards in call:
											if stack<=toCallx:return 'call'
											return 'push'

											return 'push'
									if [1 for x,y in statsOOPMas if lineIP in x[:len(lineIP)] and type(y)==type(1) and y>1] and preflopCards in push:
										if effectiveStack()-(potx+toCallx)<=1 and not stack<=toCallx and not 'is all in' in last[-1] and (effectiveStack()/potx)<=8:return 'push'
										if stack and toCallx and stack<=toCallx:return 'call'
										if 'is all in' in last[-1]:return 'call'
										return 'raise'
									if preflopCards in call:
										return 'call'
									...
									if preflopCards in powerCall:
										if 'is all in' in last[-1]:return 'call'
										if stack<=toCallx:return 'call'
										if lineIP=='rrr' or lineIP=='crr': return 'push'
										return 'raise'
								...
								if preflopCards in powerCall:
									...
									if lineIP=='rrr' or lineIP=='crr': return 'push'
									return 'raise'
								# if preflopCards in raiss and toCallx/(potx+toCallx)<=0.33:return 'call'
								
							if rounddd()==1:
								if cards in nice:
									if 'is all in' in last[-1]:return 'call'
									if stack<=toCallx:return 'call'
									if effectiveStack()/(potx+toCallx)>=1:
										if 'is all in' in last[-1]:return 'call'
										if stack<=toCallx:return 'call'
										return 'raise'
									if 'is all in' in last[-1]:return 'call'
							...
							if rounddd()>1:
								if cards in nice:
									...
									if stack<=toCallx:return 'call'
									return 'push'
								if toCallx/(potx+toCallx)<=0.25:
									if cards in nice:return 'call'
							if [1 for x,y in statsOOPMas if lineIP+'cbf' in x[:len(lineIP+'bf')] and type(y)==type(1) and y>0] and 'is all in' not in last[-1]:
									
									if toCallx/(potx-toCallx)<=4.5:
										if effectiveStack()/(potx+toCallx)>=1:return 'call'
							if rounddd()==0:
								if [1 for x,y in statsOOPMas if lineIP+'rf' in x[:len(lineIP+'bf')] and type(y)==type(1) and y>0] and 'is all in' not in last[-1]:
									
									if toCallx/(potx-toCallx)<=4.5:
										if effectiveStack()/(potx+toCallx)>=2.5:return 'raise'
										return 'push'

									if toCallx/(potx-toCallx)<=1:
										if effectiveStack()/(potx+toCallx)>=1.5:return 'raise'
										return 'push'
							if [1 for x,y in statsOOPMas if lineIP in x[:len(lineIP)] and type(y)==type(1) and y>1] and rounddd()>0 and cards in contBetRange:
								if effectiveStack()>=0.5*potx and not stack<=toCallx and not 'is all in' in last[-1]:return 'call'
								if stack<=toCallx:return 'call'
								if 'is all in' in last[-1]:return 'call'
								return 'push'
							if [1 for x,y in statsOOPMas if lineIP in x[:len(lineIP)] and type(y)==type(1) and y>1] and rounddd()==0 and preflopCards in call:
								if effectiveStack()>=potx and not stack<=toCallx and not 'is all in' in last[-1]:return 'raise'
								if stack<=toCallx:return 'call'
								if 'is all in' in last[-1]:return 'call'
								return 'push'
							if toCallx/(potx+toCallx)<=0.25:
								if cards in contBetRange:return 'call'
							return 'fold'
						if lineIP and lineIP[-1]=='b':
							if rounddd()==1:
								if cards in nice:
									if 'is all in' in last[-1]:return 'call'
									if stack<=toCallx:return 'call'
							...
							if [1 for x,y in statsOOPMas if lineIP+'rf' in x[:len(lineIP+'bf')] and type(y)==type(1) and y>0] and 'is all in' not in last[-1]:
									...
									if toCallx/(potx-toCallx)<=1:
										if effectiveStack()/(potx+toCallx)>=1:return 'raise'
										return 'push'
							if [1 for x,y in statsOOPMas if lineIP in x[:len(lineIP)] and type(y)==type(1) and y>1] and cards in contBetRange:
								if effectiveStack()>=0.5*potx and not stack<=toCallx and not 'is all in' in last[-1]:return 'call'
							if toCallx/(potx+toCallx)<=0.25:
								if cards in contBetRange:return 'call'
							return 'fold'
						if lineIP and lineIP[-1]=='0':
							if [1 for x,y in statsOOPMas if lineIP+'bf' in x[:len(lineIP+'bf')] and type(y)==type(1) and y>0]:
								if effectiveStack()>=0.5*potx: return 'raise'
								return 'push'
							if rounddd()==1:
					...
					if position()=='ip':
						if not lineIP:
							...
							if preflopCards in raiss:return 'raise'
							if [1 for x,y in statsIPMas if 'rf' in x[:len(lineIP+'bf')] and type(y)==type(1) and y>1] and 'is all in' not in last[-1]:
								
								if effectiveStack()/potx>=1:return 'raise'
								return 'push'
							return 'fold'
						if lineIP and lineIP[-1]=='r':
							if rounddd()==0:
								if toCallx/(potx-toCallx)<=4.5:
									if (effectiveStack()/potx)<=8:
										if preflopCards in powerCall and 'is all in' in last[-1]:return 'call' 
										if stack<=toCallx:return 'call'
										if preflopCards in powerCall:return 'push' 
									if preflopCards in call:
										if 'is all in' in last[-1]:return 'call'
										if stack<=toCallx:return 'call'
							...
							if rounddd()>1:
								...
								if toCallx/(potx+toCallx)<=0.25:
									if cards in contBetRange:return 'call'
							if [1 for x,y in statsIPMas if lineIP+'cbrf' in x[:len(lineIP+'cbrf')] and type(y)==type(1) and y>0]  and 'is all in' not in last[-1]:
								
								if rounddd()==0:
									if toCallx/(potx-toCallx)<=4.5:
										if effectiveStack()/(potx+toCallx)>=2.5:return 'call'
							if [1 for x,y in statsIPMas if lineIP+'c0bf' in x[:len(lineIP+'c0bf')] and type(y)==type(1) and y>0] and 'is all in' not in last[-1]:
								
								if rounddd()==0:
									if toCallx/(potx-toCallx)<=4.5:
										if effectiveStack()/(potx+toCallx)>=1:return 'call'
							if [1 for x,y in statsIPMas if lineIP in x[:len(lineIP)] and type(y)==type(1) and y>1] and preflopCards in call and rounddd()==0:return 'push'
							if [1 for x,y in statsIPMas if lineIP in x[:len(lineIP)] and type(y)==type(1) and y>1] and preflopCards in contBetRange and rounddd()>0:
								if effectiveStack()>=0.5*potx:return 'raise'
								return 'push'
							if toCallx/(potx+toCallx)<=0.25:
								if cards in contBetRange:return 'call'
						...
						if lineIP and lineIP[-1]=='b':
							...
							if rounddd()>1:
								...
								if toCallx/(potx+toCallx)<=0.25:
									if cards in contBetRange:return 'call'
							if [1 for x,y in statsIPMas if lineIP+'rf' in x[:len(lineIP+'cb')] and type(y)==type(1) and y>0] and 'is all in' not in last[-1]:
								
								if toCallx/(potx-toCallx)<=1:
									if effectiveStack()/(potx+toCallx)>=1: return 'raise'
									return 'push'
							if [1 for x,y in statsIPMas if lineIP in x[:len(lineIP)] and type(y)==type(1) and y>1] and cards in contBetRange:
								if effectiveStack()>=0.5*potx:return 'call'
						...
						if lineIP and lineIP[-1]=='0':
							...
							if rounddd()==3:
								if cards in nice:
									if effectiveStack()/potx>=1:return 'raise'
									return 'push'
							if [1 for x,y in statsIPMas if lineIP+'bf' in x[:len(lineIP+'cb')] and type(y)==type(1) and y>0]:

								if effectiveStack()>=0.5*potx:return 'raise'
								return 'push'
							if [1 for x,y in statsIPMas if lineIP in x[:len(lineIP)] and type(y)==type(1) and type(y)==type(1) and y>1] and cards in nice:
								if effectiveStack()>=0.5*potx:return 'raise'
								return 'push'
							return 'check'
						if lineIP and lineIP[-1]=='c':
							...
							if [1 for x,y in statsIPMas if lineIP+'bf' in x[:len(lineIP+'bf')] and type(y)==type(1) and y>0]:
								if effectiveStack()>=0.5*potx:return 'raise'
								return 'push'
							if [1 for x,y in statsIPMas if lineIP in x[:len(lineIP)] and type(y)==type(1) and y>1]:
								if effectiveStack()>=0.5*potx:return 'raise'
							if [1 for x,y in statsIPMas if lineIP in x[:len(lineIP)] and type(y)==type(1) and y>1] and cards in nice:
								if effectiveStack()>=0.5*potx:return 'raise'
								return 'push'
							return 'check'	
				...
				def didI():
					...
					if pos:
						if len(last)>=3:
							if '7_ee_Zy posts the' in last[-3] and 'folds'  not in last[-1]:return (ActionMyFin())
							if pos=='oop' and 'is [' in last [-1]and 'folds'  not in last[-1]:return (ActionMyFin())
							if 'raises to' in last[-1] and '7_ee_Zy' not in last[-1]and 'folds'  not in last[-1]:return (ActionMyFin())
							if 'bets' in last[-1] and '7_ee_Zy' not in last[-1]and 'folds'  not in last[-1]:return (ActionMyFin())
							if 'checks' in last[-1] and '7_ee_Zy' not in last[-1] and 'is [' in last[-2]: return  (ActionMyFin())
